...
rule get_hg38_bam:
    ...
    output:
        path_to_data + '/{cohort}/results/bam_dedupd/{sample}.aligned.sorted.filt3.dedupd_hg38only.bam',
...
rule fastqc_bam:
    input:
        path_to_data + '/{cohort}/results/bam_dedupd/{sample}.aligned.sorted.filt3.dedupd_hg38only.bam',
    output:
        html = path_to_data + '/{cohort}/qc/fastqc_bam/{sample}.aligned.sorted.filt3.dedupd_hg38only_fastqc.html',
        zipfile = path_to_data + '/{cohort}/qc/fastqc_bam/{sample}.aligned.sorted.filt3.dedupd_hg38only_fastqc.zip',
    resources: cpus=1, mem_mb=8000, time_min='24:00:00'
    params:
        outdir = lambda wildcards, output: '/'.join(output.html.split('/')[0:-1])
    conda: '../conda_env/fastqc.yml'
...
rule bam_flagstat:
    input:
        path_to_data + '/{cohort}/results/bam_dedupd/{sample}.aligned.sorted.filt3.dedupd_hg38only.bam',
    output:
        path_to_data + '/{cohort}/qc/flagstat/{sample}.aligned.sorted.filt3.dedupd_hg38only.bam.flagstat',
...
rule MarkDuplicates_QC:
    input:
        path_to_data + '/{cohort}/results/bam_dedupd/{sample}.aligned.sorted.filt3.dedupd_hg38only.bam',
    output:
        dedupd_bam = temp(path_to_data + '/{cohort}/qc/picard_qc/{sample}.picardMarkDup.bam'),
...
rule QC_out:
    input:
        ...
        fastqc_bam = path_to_data + '/{cohort}/qc/fastqc_bam/{sample}.aligned.sorted.filt3.dedupd_hg38only_fastqc.html',
        flagstat = path_to_data + '/{cohort}/qc/flagstat/{sample}.aligned.sorted.filt3.dedupd_hg38only.bam.flagstat',
        ...
        picard_dedupd_metric = path_to_data + '/{cohort}/qc/picard_qc/{sample}.marked_dup_metrics.txt',
    ...
    conda: '../conda_env/samtools.yml'
    shell:
        'cat {input.medips_qc} {input.F19K16_F24B22_methyl_summary}.parsed {input.picard_gcbias_summary}.parsed {input.picard_dedupd_metric}.parsed {input.picard_alignment_metric}.parsed {input.picard_quality_metric}.parsed > {output.full_qc} && rm {input.F19K16_F24B22_methyl_summary}.parsed {input.picard_gcbias_summary}.parsed {input.picard_dedupd_metric}.parsed {input.picard_alignment_metric}.parsed {input.picard_quality_metric}.parsed'
